...
if (isset($_POST['save'])) {
    ...
    if ($_FILES['pic']['tmp_name'] == '' || getimagesize($_FILES['pic']['tmp_name']) == FALSE) {
        echo "<center> <p id='p' style='color: green; font-size: 22px; width:98%;text-align:center;'> !!  عفوا, لم يتم اختيار صورة </p></center>";
    } else {
        $image = base64_encode(file_get_contents(addslashes($_FILES['pic']['tmp_name'])));
        //var_dump($image);   // str(0)=''
...
<html>
    ...
    <center> <h1 class="text-center"><?php echo $title; ?></h1>
        <form class="login" style="width: 60%;" method="post" enctype="multipart/form-data">
            <p id="h1" class="img" style='height:200px; width: 70%; border-radius: 30px;' name="<?php echo "img$i"; ?>"
            ...
            <input class="form-control input-lg" type="file"  name="pic"  placeholder="الصورة" autocomplete="off"
