...
<html>
    ...
    <body id="body" dir="rtl" onpageshow="disp_selec_tbl(),show_msg()" onpagehide="alert('m')">
      <center>
       ...
       // $products_global_fetsh2= prod_data_to_arr2(get_products($conn));
        ...
        $increase_arr2= get_increases($conn,$dd);
        for ($i = 0; $i < count($increase_arr2); $i++) {
            $id_p = $increase_arr2[$i][1];
            $quantitiy= $increase_arr2[$i][2];
            if($quantitiy<0){$quantitiy=$quantitiy*(-1);}
            $paid= $increase_arr2[$i][4];
            $notes='عجز مدفوع';
        ...
        if(isset($_POST["buy$id_p"])){
            ...
            $datel="2019-$month-01";
            // new_order($conn, $bill_no,$datel, $id_p,$quantitiy, $total_pricee,1,1,1,$notes, 0);
             // update_remaining($conn,$id_p,-($quantitiy));
              buy_from_inv($conn,$id_p);
          }
        if(isset($_POST["back$id_p"])){
              back_to_inv($conn,$id_p);
          }
        }
  ...
  <tr>
   ...
   <th style="width: 15%;">كمية الزيادة </th>
   <th style="width: 15%;">إجمالي الزيادة </th>
   <th style=""></th>
 </tr>
   </table> 
   </div> 
    <div id="content" class="content" style="text-align: center; width: 100%; ">
           ...
           // $products_day_fetsh= get_day_products($conn,$dd);
            $leak_arr= get_increases($conn,$dd);
    ...
    ?>
        ...
        <td style='font-weight:bold; width:25%;'>
           ...
           <td style="width: 15%; "><center>
            ...
            <td style=" "><center>
            <?php if($paid==0){ ?> 
                <input type="submit" name="<?php echo "buy$id_p";?>" class="btn" style="width:40%; font-size: 18px;" id="btn" value="بيع" />                
            <?php }else{ ?>                
                <b class="mtd" id="h1" style='color: black;'>تم البيع</b> 
                <input type="submit" name="<?php echo "back$id_p";?>" class="btn" style="width:40%; font-size: 18px;" id="btn" value="بقاء" />
              <?php } ?>
